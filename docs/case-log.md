...
## Rows from the kidnap replays, 2026-08-26

A handheld sensor kidnapped three times per sequence (view covered, carried
elsewhere), with the dataset's own continuous ground truth. Full writeup in
`finding-kidnap.md`; numbers recompute from `results/kidnap/` and
`results/kidnap02/` via `scripts/kidnap_grade.py`. The replays generate the
localiser output, disclosed as everywhere else; the occlusion windows that zone
...
| K2 | tf_jump | map correction speed | 45.3 m/s | full run | **ungraded** | 21 detections on the second sequence, where the localiser degrades (zone medians 2 to 5 m, excursions to 14.8 m) rather than settling wrong. All three kidnaps caught, the first from one frame before the occlusion threshold is crossed as the cover enters the view. Counted against it: one firing at 3.0 m/s at 2.1 s into the run, inside the estimator's own 2 s initialisation cool-time, with real error under 0.24 m. |
