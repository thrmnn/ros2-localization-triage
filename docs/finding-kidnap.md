...
One 147-second handheld sequence from the
[Hard Point Cloud Localization Dataset](https://zenodo.org/records/10122133)
(Koide et al., AIST, CC BY 4.0): an Azure Kinect carried through a building at
walking pace and kidnapped three times, meaning its view was covered while it
was carried somewhere else, twice into another room. The dataset ships its own
...
One sequence, one localiser, one platform. The `map->depth_camera_link` edge
